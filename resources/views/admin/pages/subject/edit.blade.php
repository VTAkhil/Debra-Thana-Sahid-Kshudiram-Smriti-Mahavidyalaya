            ...
            <div class="col-xl-8 col-xxl-8 col-sm-12">
                ...
                <div class="card">
                    <div class="card-header justify-content-start">
                        <h5 class="card-title me-3">Edit Subject</h5>
                        <a href="{{ url('/vtadmin/list-subject') }}" class="btn btn-primary"><i class="fa-solid fa-angles-left"></i> Back</a>
                    </div>
                    <div class="card-body">
                        <form action="{{ url("/vtadmin/editSubject",$data->id) }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label class="form-label" for="subject">Enter Subject Title</label>
                                        <input type="text" name="subject" id="subject" placeholder="Enter Subject" value="{{ $data->subject }}" class="form-control" required>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="category_id">
                                            ...
                                            <?php $categories = DB::table('categories')->where("status","1")->orderBy("id","Desc")->get(); ?>
                                                @foreach ($categories as $Category)
                                                    <option value="{{ $Category->id }}" @selected($Category->id == $data->category_id)>
